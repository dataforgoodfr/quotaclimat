...
from quotaclimat.data_processing.read_format_deduplicate import read_and_format_all_data_dump, filter_data_only_keep_top_audiance
...
def figures_computation_cop26():
    # get data 
    df_all = read_and_format_all_data_dump(path_folder = PATH_EXTRACTS_DATA_COP26, path_channel_metadata=None)
    df = filter_data_only_keep_top_audiance(df_all, PATH_CHANNEL_METADATA)
    # FIGURES
    # over time
    multiplier = 2 / (df["channel_name"].nunique() * 60 * 18) # multiplier = n_mentions * 2 min / (n_channels * 60 minutes * 20h)
    fig_mentions_over_time = show_mentions_over_time(df,freq = "D",method = multiplier,height = 500,text_auto = ".1%")
