...
from quotaclimat.utils.channels import (
        TOP_CHANNELS_RADIO,
        TOP_CHANNELS_TV_CONTINU,TOP_CHANNELS_TV_GENERALISTE
        )
...
def label_media_from_list(media,
...
def get_filtered_data(data):
    ...
    data_filtered = mt_exploration.filter_data_between_hours(data_completed,"06:00","24:00").reset_index(drop = True)
    return data_filtered


def get_media_time(data_filtered,
                    minutes_by_chunk = 2,
    ...
    media_time = (
        data_filtered.groupby(["media2"])
        .agg({"count":"sum","channel_name":"nunique"})
    )
    ...
    media_time["media_time"] = media_time["count"] * minutes_by_chunk # media_time is in minutes
    media_time["total_time"] = media_time["n_channels"] * n_days * hour_a_day * 60 # 60 : from hours to minutes
...
def get_ranking_evolution(
    ...
    ranking["media_part_total"] = ranking["count_total"] * minutes_by_chunk / ranking["total_time"]
...
def plot_volume_mediatique(media_time,COLOR_RADIO=COLOR_RADIO,COLOR_TV=COLOR_TV):
    fig = px.bar(media_time.reset_index(),x = "media2",y = "media_part",height = 400,text_auto = ".1%")
...
def plot_classement_volume_mediatique(
    ...
    fig = mt_exploration.show_mentions_by_channel(data_filtered,list_of_channels=top_channels_list,n = 25,split="keyword",
...
def media_volume_over_time(
    ...
    if n_days==1:
        PLOT_FREQUENCY = "H"
        multiplier = minutes_by_chunk / (n_channels * 60) # as if only 1 hour for the evolution plot
    else:
        PLOT_FREQUENCY = "1D"
        multiplier = minutes_by_chunk / (n_channels * hour_a_day * 60) # as if only 1 day for the evolution plot
    ...
    fig.update_layout(font_family="Poppins",
                        ...
                        xaxis_title = "Date",
                        yaxis_title="% du volume médiatique",
                        title = "Evolution du volume médiatique",legend_title = "",yaxis_tickformat='0%',
                        )
    return fig