"""
Sauvegarde des résultats de classification en base de données (version embedding)
==================================================================================
Version embedding de e06_export_classification.py.
Génère les IDs des Ads à partir des embeddings au lieu des paires de constellation maps.
"""
...
from .e04b_group_chunks import embedding_canonical
...
def _ad_id_from_embedding_chunks(chunks) -> str:
    """Generate a stable Ad ID from chunk embeddings.

    Quantizes embeddings to int16 for stability, then hashes.
    """
    all_embeddings = []
    for chunk in chunks:
        emb = chunk.fingerprint.embedding
        if emb is not None:
            # Quantize to int16 for hash stability across float rounding
            quantized = np.round(np.array(emb) * 1000).astype(np.int16).tolist()
            all_embeddings.extend(quantized)

    if not all_embeddings:
        # Fallback: hash duration + energy
        raw = json.dumps(
            [
                (c.fingerprint.duration_sec, c.fingerprint.energy_mean)
                for c in chunks
            ],
            separators=(",", ":"),
        )
    ...
    raw = json.dumps(all_embeddings, separators=(",", ":"))
    return hashlib.sha256(raw.encode()).hexdigest()[:32]
...
def embedding_database_storage_save(fragments: list[Fragment], chunk_hash: str):
    ...
    try:
        ...
        for group_id, group_fragments in groups.items():
            if any(f.classification == "already_known_ad" for f in group_fragments):
                ...
                canonical_chunks = [
                    embedding_canonical(list(position))
                    for position in chunks_by_position
                ]

                ad_id = _ad_id_from_embedding_chunks(canonical_chunks)
        ...
        for fragment in ungrouped:
            ...
            ad_id = _ad_id_from_embedding_chunks(chunks)
            fragment.group_id = ad_id
            fragment_type = (
