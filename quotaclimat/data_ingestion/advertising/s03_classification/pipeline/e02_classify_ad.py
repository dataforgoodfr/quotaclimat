...
from quotaclimat.data_ingestion.advertising.s03_classification.ad_classification_schema import build_ad_classification_model
...
from quotaclimat.data_ingestion.advertising.s03_classification.pipeline.llm_chat import chat_completion_to_dict
from quotaclimat.data_ingestion.advertising.s03_classification.pipeline.runner import StageResult, run_stage
from quotaclimat.data_ingestion.advertising.s03_classification.prompts import load_prompt
from quotaclimat.data_ingestion.advertising.s03_classification.settings import VLMSettings
from quotaclimat.data_ingestion.advertising.s03_classification.taxonomy import SECTOR_LIST, dict_sectors
...
def _build_messages(system_prompt: str, user_prompt_tpl: str, transcript: str | None,
...
def make_process_one(
    s3_client, s3_config, llm_client, model_name, system_prompt, user_prompt_tpl,
    schema, transcripts, ttl, llm_timeout, retries,
):
    def process_one(ad_id: str) -> StageResult:
        try:
            url = get_presigned_url_for_ad(
                ad_id=ad_id, extension="mp4", ttl=ttl,
                config=s3_config, s3_client=s3_client,
            )
        ...
        messages = _build_messages(system_prompt, user_prompt_tpl, transcripts.get(ad_id), url)
        try:
            parsed = chat_completion_to_dict(
                llm_client, model=model_name, messages=messages,
                schema=schema, timeout=llm_timeout, retries=retries,
                schema_name="ad_classification",
            )
        except ValueError as e:
            return StageResult(
                ad_id=ad_id, status="ad_type_parse_error",
                payload={"parsed": None}, error=f"parse: {e}",
            )
        except Exception as e:
            return StageResult(ad_id=ad_id, status="error", error=f"llm: {e}")

        return StageResult(
            ad_id=ad_id, status="ad_type_done",
            payload={
                "parsed": parsed,
...
def make_flush(session_factory, model_name: str):
    def flush(results: list[StageResult]) -> None:
        ...
        for r in results:
            ...
            ok_rows.append({
                "b_id": r.ad_id,
                "b_fragment_type": ((r.payload or {}).get("parsed") or {}).get("content_type", "advertising"),
                "b_prediction": [entry],
    ...
    try:
        return run_stage(
            ...
            process_one=make_process_one(
                s3_client, s3_config, llm_client, model_settings.model_name,
                system_prompt, prompt.user_prompt, schema, transcripts,
                ttl, llm_timeout, retries,
            ),
...
if __name__ == "__main__":
    ...
    run(
        ...
        prompt_path=os.environ.get("PROMPT_PATH", "quotaclimat/data_ingestion/advertising/s03_classification/prompts/ad_type.yaml"),
    )
