...
from quotaclimat.data_ingestion.advertising.s03_classification.taxonomy import dict_sectors, get_subcategory_list
...
def select_pending(engine, limit: int | None) -> list[dict]:
    ...
    with Session() as session:
        stmt = (
            ...
            .where(Ad.prediction_status.in_(
        ...
        rows: list[dict] = []
        for ad_id, prediction, transcript, sector, input_status in session.execute(stmt).all():
...
def _build_messages(system_prompt: str, user_prompt_tpl: str, row: dict, video_url: str) -> list[dict]:
...
def make_process_one(
    ...
    rows_by_id: dict[str, dict], ttl, llm_timeout, retries,
):
    def process_one(ad_id: str) -> StageResult:
        ...
        messages = _build_messages(system_prompt, user_prompt_tpl, rows_by_id[ad_id], url)
        try:
            parsed: dict[str, Any] = chat_completion_to_dict(
                llm_client, model=model_name, messages=messages,
                schema=None, timeout=llm_timeout, retries=retries,
            )
        except ValueError as e:
            return StageResult(
                ad_id=ad_id, status="subcat_parse_error",
        ...
        return StageResult(
            ad_id=ad_id, status="subcat_done",
    ...
    try:
        return run_stage(
            ...
            process_one=make_process_one(
                ...
                prompt.system_prompt, prompt.user_prompt, rows_by_id,
...
if __name__ == "__main__":
    ...
    run(
        ...
        prompt_path=os.environ.get("PROMPT_PATH", "quotaclimat/data_ingestion/advertising/s03_classification/prompts/subcat.yaml"),
    )