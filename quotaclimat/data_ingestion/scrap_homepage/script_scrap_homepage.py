...
from quotaclimat.data_ingestion.scrap_homepage.config_homepage_website import config_homepage_lemonde
...
def get_home_content():
    ...
    soup = BeautifulSoup(
        requests.get('https://www.lemonde.fr/', headers={'User-Agent': 'Mozilla/5.0'}).content, "html.parser")
...
def crawl_sections(section: str, config: dict) -> List[dict]:
    ...
    for position, unique_class in enumerate(find_all_classes, 1):
        ...
        link_by_class.extend(
            decomposer_class(section, config, unique_class,
                             find_all_classes, homepage_content, position)
        )

    return link_by_class


def decomposer_class(section: str, config: dict, unique_class: Tag, find_all_classes: List[ResultSet],
                     homepage_content: BeautifulSoup, position: int) -> List[dict]:
    ...
    if config["sub_container"]["tag"] is None:
        ...
        return [
            ...
            for count_li, li in enumerate(unique_class.findAll(config["sub_container"]["tag"]), 1)
        ]

    elif config["sub_container"]["tag"] == "a":
        find_all_classes_str = str(find_all_classes)
        regex_find_classes = re.findall(
            r'article--top old__top-article old__top-article-[0-9]+', find_all_classes_str)
        return [
            {
                "url": homepage_content.findAll(config["sub_container"]["tag"], class_=class_)[0]['href'],
                "section": section,
...
def get_df_article() -> pd.DataFrame:
    ...
    for section, config in config_homepage_lemonde.items():
        try:
            ...
            logging.error(
                "[CRAWL_HOME_LE_MONDE][ERROR] SECTION %s failed for reason: %s", section, err)
...
def get_type_home(df):
    ...
    if (df['section'] == 'main_title_related').any() and (
            df['section'] == 'routine_main_title_headlines').any():
        ...
        return df
            ...
            df['section'] == 'classic_main_title_headlines').any():
