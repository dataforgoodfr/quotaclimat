...
def retrieve_channel_infos(medias: list) -> dict:
	...
	for media in medias:
		...
		channel_info['uploads_id'] = response_bis['contentDetails']['relatedPlaylists']['uploads']
...
def converting_to_csv(final: dict, media: str):
	...
	date = datetime.date.today() # A voir pour la date d'exécution du script si la fonction today() ne pose pas problème
	landing_path = (
        "data_public/youtube_dumps/channel=%s/year=%s/month=%s/"
        %(media, date.year, date.month)
	)
	if not os.path.exists(landing_path):
		os.makedirs(landing_path)
	csv = df.to_csv(landing_path+"%s.csv" % date.strftime("%Y%m%d"))
...
def full_scraping():
	...
	for media in CHANNEL_CONFIG:
		...
		date = datetime.date.today() # Même chose que pour la fonction 'converting_to_csv'
