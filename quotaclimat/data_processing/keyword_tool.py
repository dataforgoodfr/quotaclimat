...
class KeywordsTool:
    ...
    def load_from_dict(self,keywords:dict):
        ...
        self.keywords = {k:list(set(v+remove_symbol(v))) for k,v in self.keywords.items()}
        self.keywords = {k:list(set(v+add_singular_and_plural(v))) for k,v in self.keywords.items()}
        self.keywords = {k:[x for x in v if len(x) > 2] for k,v in self.keywords.items()}

        if self.lowercase:
            self.keywords = {k:list(set([x.lower() for x in v])) for k,v in self.keywords.items()}
    ...
    def load_from_airtable(self,airtable_api_key = None,airtable_base_id = None,airtable_table_name = None,keyword_col = "name",variants_col = "variants"):

        if airtable_api_key is not None:

            # Load data from Airtable using PyAirtable
            self.airtable = Table(airtable_api_key,airtable_base_id,airtable_table_name)
            self.data = pd.DataFrame([x["fields"] for x in self.airtable.all()]).dropna(subset = [keyword_col]).set_index(keyword_col)
            ...
            self.load_from_airtable(
                airtable_api_key = os.environ["AIRTABLE_API_KEY"] if airtable_api_key is None else airtable_api_key,
                airtable_base_id = os.environ["AIRTABLE_BASE_ID"] if airtable_base_id is None else airtable_base_id,
                airtable_table_name = os.environ["AIRTABLE_TABLE_NAME"] if airtable_table_name is None else airtable_table_name,
    ...
    def count_keywords(self,text,as_dict = False,as_category = True):
        count = (self.extract_keywords(text,span_info = True,as_dataframe = True)
    ...
    def count_keywords_on_corpus(self,texts,as_melted = False,as_category = False):
        ...
        if as_melted:
            ...
            for i,text in enumerate(tqdm(texts)):
                ...
                text_count = self.count_keywords(text)[["category","count"]].reset_index()
    ...
    def count_keywords_by_paragraph(self,text):
        ...
        for i,paragraph in enumerate(paragraphs):
            if len(paragraph) > 3:
                keywords = self.extract_keywords(paragraph,span_info = True,as_dataframe = True)
        ...
        count = count.assign(count = lambda x : 1).groupby(["keyword","paragraph_number"],as_index = False)["count"].sum().set_index("keyword")
    ...
    def show_keywords(self,text,jupyter = True):
        ...
        if isinstance(text,str):
            ...
            keywords = [{"start":keyword[1],"end":keyword[2],"label":keyword[0]} for keyword in keywords]
            ...
            # Render Displacy
            return displacy.render(data,style = "ent",manual = True,jupyter = jupyter,options = options)
