...
class NLPFilteringModel:
    def __init__(self):
        ...
        self.category_loaded_model = AutoModelForSequenceClassification.from_pretrained(
            self.model_name
        )
        self.category_predictor = TextClassificationPipeline(
            model=self.category_loaded_model,
            tokenizer=self.category_loaded_tokenizer,
    ...
    def dedup_scores(self, scores):
        ...
        scores = (
            scores.sort_values(["sentence_id", "score"], ascending=[True, False])
            .reset_index(drop=True)
            .drop_duplicates(subset=["sentence_id"])
            .reset_index(drop=True)
        )

        scores["sentence_percentage"] = (
            scores["sentence_len"] / scores["sentence_len"].sum()
        )
    ...
    def show_scores(self, scores, height=400, **kwargs):
        return px.bar(
            scores, x="sentence_id", y="score", color="label", height=height, **kwargs
        )
