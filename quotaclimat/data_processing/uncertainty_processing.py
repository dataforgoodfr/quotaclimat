...
def get_moment_of_the_day(date_test):
    ...
    return datetime.timedelta(hours = date_test.hour,
                                minutes=date_test.minute,
                                seconds=date_test.second) / datetime.timedelta(hours=24)
...
def get_formatted_score_evolution(score_evol = SCORE_EVOLUTION):
    ...
    score = pd.concat((score,pd.DataFrame(data = [[1,score.score.iloc[0]]],
                                columns= ['heure','score'])),ignore_index=True)
...
def get_score_time(date_test,formatted_score):
    ...
    return np.interp(moment_of_the_day,
                    score.heure,
                    score.score)
...
def get_score_range(range_test,score_evol = SCORE_EVOLUTION):
    ...
    mask_time = score.reset_index().heure.between(get_moment_of_the_day(range_test[0]), 
            get_moment_of_the_day(range_test[1]), 
            inclusive = "neither")

    start_score = pd.Series(data = [get_score_time(range_test[0],score)],
                        index = [get_moment_of_the_day(range_test[0])])

    end_score = pd.Series(data = [get_score_time(range_test[1],score)],
                            index = [get_moment_of_the_day(range_test[1])])

    if mask_time.sum() > 0: # il faut ajouter au moins un point
        score_time  = pd.concat((start_score,
                                score[score.index[mask_time]],
                                end_score)
            ,ignore_index=False)
...
def get_coverage_data(data,chunk_size = CHUNK_SIZE):
    coverage_data = data[['index','channel','date','keyword','mentions','n_mentions']].copy()
...
def get_mentions_date(data,chunk_size = CHUNK_SIZE):
    return data.apply(lambda x : mention_date_from_serie(x,chunk_size)
                    ,axis=1)
...
def get_mention_timestamp_by_channel(cov_data_channel,channel_name):
    ...
    mention_timestamp = sorted(list(set([x  
                for date_list in cov_data_channel['mentions_date'].values 
                    for x in date_list]))) # get in a unique list
...
def get_coverage_by_chunks(cov_data_channel,channel_name):
    data_ranges = [(row[1].date,row[1].date_end) for row in cov_data_channel.iterrows()] 
    sorted_unique_ranges = sorted(list(set(data_ranges)))
    chunks_method_initiale = pd.DataFrame(merge_date_ranges(sorted_unique_ranges)
                ,columns = ['date','date_end'])
    chunks_method_initiale['channel'] = channel_name
    return chunks_method_initiale


def get_coverage_by_window(cov_data_channel,channel_name, 
        continuity_window_minutes,mention_extension_seconds):
    ...
    chunks_data = join_timestamps_within_rolling_window(mention_timestamp,
                continuity_window,
                mention_extension)
...
def join_timestamps_within_rolling_window(mention_timestamp,continuity_window,mention_extension):
    """
    ...
    data_ranges = [(row[1].mention_windows_start,row[1].mention_windows_end) for row in df.iterrows()] 

    chunks_df = pd.DataFrame(merge_date_ranges(data_ranges)
                ,columns = ['mention_windows_start','mention_windows_end'])

    chunks_df['date'] =  chunks_df.mention_windows_start + continuity_window/2 - mention_extension / 2
    chunks_df['date_end'] =  chunks_df.mention_windows_end - continuity_window/2 + mention_extension / 2
...
def plot_grouped_chunks_and_mentions(chunks_data,df_event):
    ...
    fig1 = px.timeline(chunks_data
                ,x_start='date',x_end='date_end'
                ,y = 'channel' 
                )
  
    fig2 = px.scatter(df_event
                    ,x='x', y = 'y'

                    ,color_discrete_sequence=['#DC3912'] # forced to red
