...
MACROCATEGORY_COLUMNS = {
    ...
    "english": [
        "keyword", "is_empty", "general", "agriculture", "transport",
        "buildings", "energy", "industry", "water", "ecosystem", "resource_economy",
    ],
}
...
class DictionaryProcessor:
    ...
    def __init__(self, source: str, sheet_name: str, file_language: str, dictionary_language: str):
        ...
        self.category_column = DICTIONARY_COLUMNS[file_language][1]
        self.dictionary_language = dictionary_language

    def process(self):
        THEME_KEYWORDS = {}
        for idx, row in self.df.iterrows():
            category = row.get(DICTIONARY_CATEGORIES[self.file_language][-1], "")
            crisis_climate = row[DICTIONARY_COLUMNS[self.file_language][3]] == DICTIONARY_CATEGORIES[self.file_language][0]
            crisis_biodiversity = row[DICTIONARY_COLUMNS[self.file_language][3]] == DICTIONARY_CATEGORIES[self.file_language][1]
            crisis_resource = row[DICTIONARY_COLUMNS[self.file_language][3]] == DICTIONARY_CATEGORIES[self.file_language][2]
            ...
            if(row[self.theme_column] not in THEME_KEYWORDS):
                print(f"Found new theme {row[self.theme_column]}")
                THEME_KEYWORDS[row[self.theme_column]] = []
            THEME_KEYWORDS[row[self.theme_column]].append(
                {
                    "keyword": row["keyword"],
                    "category": category,
    ...
    def process(self):
        ...
        for _, row in df.iterrows():
            ...
            record = dict(
                ...
                is_empty=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][1], False)), # is_empty
                general=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][2], False)), # general
                agriculture=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][3], False)), # agriculture
                transport=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][4], False)), # transport
                batiments=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][5], False)), # batiments
                energie=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][6], False)), # energie
                industrie=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][7], False)), # industrie
                eau=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][8], False)), # eau
                ecosysteme=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][9], False)), # ecosysteme
                economie_ressources=bool(row.get(MACROCATEGORY_COLUMNS[self.file_language][10], False)), # economie_ressources
...
class i18nDictionaryProcessor:
    ...
    def process(self):
        ...
        for index, row in df.iterrows():
            ...
            keyword_english = None if pd.isna(row.get(self.english)) else TranslatedKeyword(self.english.lower(),row.get(self.english)) 
            keyword_german = None if pd.isna(row.get(self.german)) else TranslatedKeyword(self.german.lower(),row.get(self.german)) 
            keyword_spanish = None if pd.isna(row.get(self.spanish)) else TranslatedKeyword(self.spanish.lower(),row.get(self.spanish)) 
            keyword_portuguese = None if pd.isna(row.get(self.portuguese)) else TranslatedKeyword(self.portuguese.lower(),row.get(self.portuguese)) 
            keyword_polish = None if pd.isna(row.get(self.polish)) else TranslatedKeyword(self.polish.lower(),row.get(self.polish)) 
            keyword_danish = None if pd.isna(row.get(self.danish)) else TranslatedKeyword(self.danish.lower(),row.get(self.danish))  
            keyword_italian = None if pd.isna(row.get(self.italian)) else TranslatedKeyword(self.italian.lower(),row.get(self.italian)) 
            keyword_arabic = None if pd.isna(row.get(self.arabic)) else TranslatedKeyword(self.arabic.lower(),row.get(self.arabic)) 
            keyword_greek = None if pd.isna(row.get(self.greek)) else TranslatedKeyword(self.greek.lower(),row.get(self.greek)) 
            keyword_latvian = None if pd.isna(row.get(self.latvian)) else TranslatedKeyword(self.latvian.lower(),row.get(self.latvian)) 

            if(theme_name not in THEME_KEYWORDS):
                print(f"Adding theme {row['Category_legacy']} - {theme_name}")
                THEME_KEYWORDS[theme_name] = []
            keywords_list = [keyword_english, keyword_german, keyword_spanish, keyword_portuguese, keyword_polish, keyword_danish, keyword_italian, keyword_arabic, keyword_greek, keyword_latvian]
...
themes = set(list(french_keywords.keys()) + list(dutch_keywords.keys()) + list(i18n_keywords.keys()))
theme_keywords = {}
for theme in themes:
    theme_keywords[theme] = sorted(french_keywords.get(theme, []) + dutch_keywords.get(theme, []) + i18n_keywords.get(theme, []), key=lambda x: x["keyword"])
...
with open(output_file, 'w', encoding='utf-8') as f:
    ...
    f.write("THEME_KEYWORDS = " + json.dumps(theme_keywords, ensure_ascii=False, indent=4).replace("null", "None").replace("true", "True").replace("false", "False"))
...
with open(output_file_macro_category, 'w', encoding='utf-8') as f:
    logging.info(f"Json written  - {len(macro_categories)} keywords inside {output_file_macro_category}")
    print(f"Json written {len(macro_categories)} in {output_file_macro_category}")
    f.write("# Auto-generated from Excel file\n")
    f.write("MACRO_CATEGORIES = " + json.dumps(macro_categories, ensure_ascii=False, indent=4).replace("null", "None").replace("true", "True").replace("false", "False"))
